...
import productDetailsFakerFactory from "./productDetailsFakerFactory";
...
const ProductDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("productDetails").create(elem));
...
export default connect(mapState, mapDispatch)(ProductDetailsFakerDialogComponent);
